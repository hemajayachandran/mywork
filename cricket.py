class CricketTeam:
	def __init__(self,list):
		self.list = list
	def total_players(list,role):
		rolewise_list =[]
		for player in range(0,len(list)):
			if list[player]["Role"] == role:
				rolewise_list.append(list[player])
		return rolewise_list
				
	def team(list,role,number):
		total_roles=CricketTeam.total_players(list,role)
		best_roles,outer_index,inner_index = [],0,1
		if role == "Opener" or role == "TopOrder" or role == "MiddleOrder" or role == "WK":
			while(True):
				if total_roles[outer_index]["BatSkill"] > total_roles[inner_index]["BatSkill"]:
					inner_index+=1
				elif total_roles[outer_index]["BatSkill"] < total_roles[inner_index]["BatSkill"]:
						outer_index = inner_index
						inner_index+=1
				else:
					if total_roles[outer_index]["BowlSkill"] > total_roles[inner_index]["BowlSkill"] and role != "WK":
						inner_index+=1
					elif total_roles[outer_index]["BowlSkill"] < total_roles[inner_index]["BowlSkill"] and role != "WK":
						outer_index = inner_index
						inner_index+=1
					else:
						inner_index+=1
				if inner_index == len(total_roles):
					best_roles.append((total_roles[outer_index]["Name"],total_roles[outer_index]["Role"]))
					total_roles.remove(total_roles[outer_index])
					outer_index,inner_index = 0,1
				if len(best_roles) == number:
					break
		elif role == "AllRounder":
			while(True):
				if (total_roles[outer_index]["BatSkill"]+total_roles[outer_index]["BowlSkill"])/2 > (total_roles[inner_index]["BatSkill"]+total_roles[inner_index]["BowlSkill"])/2:
					inner_index+=1
				elif (total_roles[outer_index]["BatSkill"]+total_roles[outer_index]["BowlSkill"])/2 < (total_roles[inner_index]["BatSkill"]+total_roles[inner_index]["BowlSkill"])/2:
					outer_index = inner_index
					inner_index+=1
				else:
					if total_roles[outer_index]["BowlSkill"] > total_roles[inner_index]["BowlSkill"]:
						inner_index+=1
					elif total_roles[outer_index]["BowlSkill"] < total_roles[inner_index]["BowlSkill"]:
						outer_index = inner_index
						inner_index+=1
					else:
						inner_index+=1
				if inner_index == len(total_roles):
					best_roles.append((total_roles[outer_index]["Name"],total_roles[outer_index]["Role"]))
					total_roles.remove(total_roles[outer_index])
					outer_index,inner_index = 0,1
				if len(best_roles) == number:
					break
		else:
			while(True):
				if total_roles[outer_index]["BowlSkill"] > total_roles[inner_index]["BowlSkill"]:
					inner_index+=1
				elif total_roles[outer_index]["BowlSkill"] < total_roles[inner_index]["BowlSkill"]:
					outer_index = inner_index
					inner_index+=1
				else:
					if (total_roles[outer_index]["BatSkill"]+total_roles[outer_index]["BowlSkill"])/2 > (total_roles[inner_index]["BatSkill"]+total_roles[inner_index]["BowlSkill"])/2:
						inner_index+=1
					elif (total_roles[outer_index]["BatSkill"]+total_roles[outer_index]["BowlSkill"])/2 < (total_roles[inner_index]["BatSkill"]+total_roles[inner_index]["BowlSkill"])/2:
						outer_index = inner_index
						inner_index+=1
					else:
						inner_index+=1
				if inner_index == len(total_roles):
					best_roles.append((total_roles[outer_index]["Name"],total_roles[outer_index]["Role"]))
					total_roles.remove(total_roles[outer_index])
					outer_index,inner_index = 0,1
				if len(best_roles) == number:
					break		
		return best_roles
	def players(self,list):
		final,final_list = [],[]
		final_list=CricketTeam.team(self.list,"Opener",2)
		final.append(final_list)
		final_list=CricketTeam.team(self.list,"TopOrder",2)
		final.append(final_list)
		final_list=CricketTeam.team(self.list,"MiddleOrder",2)
		final.append(final_list)
		final_list=CricketTeam.team(self.list,"AllRounder",1)
		final.append(final_list)
		final_list=CricketTeam.team(self.list,"WK",1)
		final.append(final_list)
		final_list=CricketTeam.team(self.list,"Bowler",3)
		final.append(final_list)
		return final
list=[
  {
    "Name": "Player1",
    "Role": "Opener",
    "BatSkill": 10,
    "BowlSkill": 0
  },
  {
    "Name": "Player2",
    "Role": "TopOrder",
    "BatSkill": 60,
    "BowlSkill": 0
  },
  {
    "Name": "Player3",
    "Role": "AllRounder",
    "BatSkill": 50,
    "BowlSkill": 15
  },
  {
    "Name": "Player4",
    "Role": "Bowler",
    "BatSkill": 41,
    "BowlSkill": 60
  },
  {
    "Name": "Player5",
    "Role": "AllRounder",
    "BatSkill": 44,
    "BowlSkill": 25
  },
  {
    "Name": "Player6",
    "Role": "Bowler",
    "BatSkill": 10,
    "BowlSkill": 79
  },
  {
    "Name": "Player7",
    "Role": "Bowler",
    "BatSkill": 20,
    "BowlSkill": 55
  },
  {
    "Name": "Player8",
    "Role": "Bowler",
    "BatSkill": 30,
    "BowlSkill": 55
  },
  {
    "Name": "Player9",
    "Role": "TopOrder",
    "BatSkill": 51,
    "BowlSkill": 0
  },
  {
    "Name": "Player10",
    "Role": "Opener",
    "BatSkill": 55,
    "BowlSkill": 0
  },
  {
    "Name": "Player11",
    "Role": "MiddleOrder",
    "BatSkill": 46,
    "BowlSkill": 15
  },
  {
    "Name": "Player12",
    "Role": "TopOrder",
    "BatSkill": 46,
    "BowlSkill": 0
  },
  {
    "Name": "Player13",
    "Role": "WK",
    "BatSkill": 60,
    "BowlSkill": 0
  },
  {
    "Name": "Player14",
    "Role": "MiddleOrder",
    "BatSkill": 46,
    "BowlSkill": 20
  },
  {
    "Name": "Player15",
    "Role": "WK",
    "BatSkill": 50,
    "BowlSkill": 0
  },
  {
    "Name": "Player16",
    "Role": "Opener",
    "BatSkill": 42,
    "BowlSkill": 0
  },
  {
    "Name": "Player17",
    "Role": "MiddleOrder",
    "BatSkill": 46,
    "BowlSkill": 0
  },
  {
    "Name": "Player18",
    "Role": "Opener",
    "BatSkill": 70,
    "BowlSkill": 0
  }
]	
team = CricketTeam(list)
print(team.players(list))
